...
DARK_COLORS = {
    'clean': {
        ...
        'click_bg': "#4b4b4b",
        'border_color': "#ff0000",
        'hover_border': "#ff0000",  # Keeping hover blue
        'click_border': "ff0000"   # Keeping click blue
    },
    'retro': {
        'label_bg': "#1f1f1f",
        'hover_bg': "#1f1f1f",
...
class FancyButton:
    ...
    def initialize_custom_styles(self):
        ttk_widgets = ['TButton', 'TCheckbutton', 'TCombobox', 'TEntry', 'TFrame', 'TLabel', 'TLabelframe', 
                       'TMenubutton', 'TNotebook', 'TPanedwindow', 'TRadiobutton', 'TSpinbox', 'Treeview']

        for widget in ttk_widgets:
            try:
                layout = self.style.layout(widget)
                if layout:
                    self.style.layout(f'Custom.{widget}', layout)
                self.style.configure(f'Custom.{widget}', )
            except tk.TclError:
                print(f"Warning: Could not create layout for {widget}")

        # Special handling for widgets without standard layouts
        for widget in ['TProgressbar', 'TScale', 'TScrollbar']:
            self.style.configure(f'Custom.{widget}', )
    ...
    def update_style(self, state):
        ...
        self.style_descendants(self.surface, colors, mode)

    def style_descendants(self, widget, colors, mode):
        bg_color = self.inner_frame.cget("bg")
    ...
    def style_ttk_widget(self, widget, bg_color, fg_color):
        widget_class = widget.winfo_class()
        style_name = f"Custom.{widget_class}"

        if isinstance(widget, ttk.Label):
            font_size = self.get_font_size(widget)
            self.style.configure(style_name, background=bg_color, foreground=fg_color, font=("TkDefaultFont", font_size))
            ...
            self.style.configure(style_name, troughcolor=bg_color)
        else:
            self.style.configure(style_name, background=bg_color, foreground=fg_color)
...
class FancyButtonTestApp:
    def __init__(self, root):
        ...
        self.toolbox_frame.pack(expand=False, fill=tk.BOTH)  # Changed expand to False
