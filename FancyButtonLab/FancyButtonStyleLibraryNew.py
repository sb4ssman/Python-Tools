...
LIGHT_COLORS = {
    'clean': {
        'label_bg': "#e1e1e1",
        'hover_bg': "#c7e0f4", # e5f1fb
        'click_bg': "#a9d1f5", #cce47f
        'border_color': "#adadad",
        'hover_border': "#0078d7",
        'click_border': "#005499",
        'text_bg': "#ffffff",
...
class FancyButtonStyleLibrary:
    def __init__(self):
        ...
        print(f"FancyButtonStyleLibrary initialized from {self.__class__.__module__}")
        # pass # Lol.
...
##############################
    ...
    def style_widget(self, widget, theme, mode, state):
        ...
        if isinstance(widget, tk.Listbox):
            self.style_tk_listbox(widget, colors['text_bg'], colors['text_color'], theme, mode)
        elif isinstance(widget, (tk.Label, tk.Button, tk.Entry, tk.Text)):
            widget.configure(
            ...
            if widget_class == 'TFrame':
                self.style_ttk_frame(widget, colors['label_bg'], theme, mode)
                ...
                self.style.configure(style_name, background=colors['border_color'])
