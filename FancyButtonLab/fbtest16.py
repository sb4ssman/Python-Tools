...
class FancyButton:
    ...
    def bind_children(self, widget):
        for child in widget.winfo_children():
            ...
            if not isinstance(child, (ttk.Entry, ttk.Spinbox, ttk.Combobox, ttk.Scale, ttk.Checkbutton)):
                ...
                child.bind("<Button-1>", lambda e, w=child: self.on_interactive_widget_click(e, w))
                child.bind("<ButtonRelease-1>", lambda e, w=child: self.on_interactive_widget_release(e, w))
    ...
    def on_enter(self, event):
        if event.widget.winfo_toplevel() == self.outer_frame.winfo_toplevel():
    ...
    def on_enter_child(self, event, widget):
        if event.widget.winfo_toplevel() == self.outer_frame.winfo_toplevel() and not self.is_hovered:
            self.is_hovered = True
            self.update_style("Hover")

    def on_leave_child(self, event, widget):
        if event.widget.winfo_toplevel() == self.outer_frame.winfo_toplevel() and not self.is_mouse_within_bounds(event.x_root, event.y_root):
            self.is_hovered = False
            self.update_style("")
    ...
    # it's possible to pass on canvas details
    ...
    def style_ttk_widget(self, widget, bg_color, fg_color, state):
        ...
        if widget_class in ['TLabel', 'TCheckbutton', 'TRadiobutton', 'TFrame', 'TLabelframe']:
            ...
            self.style.configure(widget_class, background=bg_color, foreground=fg_color)
            
        elif widget_class in ['TEntry', 'TSpinbox', 'TCombobox']:
            field_bg = '#ffffff' if theme == "retro" and mode == "light" else bg_color
            self.style.configure(widget_class, fieldbackground=field_bg, background=bg_color, foreground=fg_color)
...
class FancyButtonTestApp:
    ...
    def create_complex_button_surface(self, surface):
        ...
        ttk.Label(surface, text="Inner button\npresses outer.", name="complex_label_font10").pack(expand=True, fill=tk.BOTH)
    ...
    def create_prompt_surface(self, surface):
        self.pc_title = ttk.Label(surface, text="Prompt Chooser", anchor="center", name="pc_title_font12")
        ...
        self.pc_admin_check = ttk.Checkbutton(self.pc_bottom_frame, text="Admin", variable=self.admin_var, name="pc_admin_check_font10")
