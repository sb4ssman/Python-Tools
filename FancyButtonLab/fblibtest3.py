# (20) cleanup and polish 19


# ISSUES:

    # CLEAN:
        # mouse over imbedded frame highlights other frames!
    ...
    # Dark modes: 
        ...
        # Nested buttons need to take on dark mode




    
# These imports are for the test app below; not necessary for FancyButton Class
import subprocess
...
# This class supports and requires both tk and ttk
# These must be present in your imports:
import re
import tkinter as tk
from tkinter import ttk
# Import Style Library
from FancyButtonStyleLibraryNew import LIGHT_COLORS, DARK_COLORS, FancyButtonStyleLibrary
...
class FancyButton:
    def __init__(self, parent, create_surface, theme_var, dark_mode_var, command):
        ...
        self.style_library = FancyButtonStyleLibrary()
        print(f"FancyButton initialized with {self.style_library.__class__.__name__} from {self.style_library.__class__.__module__}")
    ...
    #             font_size = self.style_library.get_font_size(widget) # use get_font_size from library
    ...
    def create_surface(self, surface):
            ...
            for widget in widgets:
                try:
                    self.style_library.style_widget(widget, bg_color, fg_color, theme, mode)
    ...
    def bind_children(self, widget):
        for child in widget.winfo_children():
            ...
            # EXCLUDE: interacting with these widgets in this set will NOT trigger the click event for the fancybutton            
            ...
            # INCLUDE: specifically include interaction
            ...
            # For nested buttons (tested: 1 layer)
    ...
    def update_style(self, state):
        print(f"Entering update_style with state: {state}")
        theme = self.theme_var.get().lower()
        mode = "dark" if self.dark_mode_var.get() else "light"
        colors = DARK_COLORS[theme] if mode == "dark" else LIGHT_COLORS[theme]
        print(f"Theme: {theme}, Mode: {mode}")
        ...
        print(f"Calling self.style_library.style_widget with: surface={self.surface}, theme={theme}, mode={mode}, state={state}")
        print(f"Type of self.style_library: {type(self.style_library)}")
        self.style_library.style_widget(self.surface, theme, mode, state)
        print("self.style_library.style_widget call completed")
            
        print(f"About to call style_descendants with: surface={self.surface}, bg_color={bg_color}, fg_color={fg_color}, theme={theme}, mode={mode}, state={state}")
        self.style_descendants(self.surface, bg_color, fg_color, theme, mode, state)
        print("style_descendants call completed")
        print("Exiting update_style")

    def style_descendants(self, widget, bg_color, fg_color, theme, mode, state):
        colors = DARK_COLORS[theme] if mode == "dark" else LIGHT_COLORS[theme]
        ...
        for child in widget.winfo_children():
            if isinstance(child, tk.Listbox):
                self.style_library.style_tk_listbox(child, bg_color, fg_color, theme, mode)
            elif isinstance(child, (tk.Label, ttk.Label)):
                self.style_library.style_widget(child, theme, mode, state)
            elif isinstance(child, (tk.Frame, ttk.Frame)):
                if isinstance(child, tk.Frame):
                    child.configure(bg=bg_color)
                else:  # ttk.Frame
                    style_name = f"{child}.TFrame"
                    self.style_library.style.configure(style_name, background=bg_color)
                    child.configure(style=style_name)
            else:
                self.style_library.style_widget(child, theme, mode, state)
            
            if hasattr(child, 'winfo_children'):
                self.style_descendants(child, bg_color, fg_color, theme, mode, state)
    ...
    def recreate(self):
        try:
            ...
            if grid_info:
                ...
                self.outer_frame.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.BOTH)
        except Exception as e:
            print(f"Error recreating FancyButton: {e}")


    def recreate_buttons(self):
        print("Recreating buttons")  # Debug print
        for button in self.buttons:
            button.recreate()
        print("Finished recreating buttons")  # Debug print
...
class FancyButtonTestApp:
    def __init__(self, root):
        ...
        self.special_buttons.append(FancyButton(self.extra_examples_frame, 
                    ...
                    self.theme_var, self.dark_mode_var, self.example_label_click))
        self.special_buttons[0].outer_frame.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.BOTH)
        
        # Example 2: LabelFrame as a FancyButton
        self.special_buttons.append(FancyButton(self.extra_examples_frame,
                    self.create_labelframe_button_surface,
                    self.theme_var, self.dark_mode_var, self.labelframe_click))
        self.special_buttons[1].outer_frame.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.BOTH)
        
        # Example 3: Multiple buttons in a FancyButton
        self.special_buttons.append(FancyButton(self.extra_examples_frame,
                    self.create_complex_button_surface,
                    self.theme_var, self.dark_mode_var, self.complex_button_click))
        self.special_buttons[2].outer_frame.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.BOTH)
    ...
    def create_prompt_surface(self, surface):
        ...
        self.pc_bottom_frame = tk.Frame(surface, name="pc_bottom_frame")
        ...
        self.pc_prompt = ttk.Label(self.pc_bottom_frame, text="OPEN", anchor="center", name="pc_prompt_font12")
    ...
    def create_listbox_surface(self, surface):
        ...
        frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.lb_items = tk.Listbox(frame, width=8, height=5, name="lb_items_font8")
    ...
    def recreate_special_buttons(self,button):
        grid_info = button.outer_frame.pack_info()
        button.outer_frame.destroy()
        button.__init__(button.parent,button.create_surface,button.theme_var,button.dark_mode_var,button.command)
        button.outer_frame.pack(grid_info)

    # Update the app's style between the 4 states: Retro/Clean-Light/Dark; MUST call fancybuttons to redraw
    def update_app_style(self):
        ...
        for button in self.buttons + self.special_buttons:
            button.update_style("")
