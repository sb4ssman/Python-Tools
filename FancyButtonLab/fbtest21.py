...
class FancyButton:
    ...
    def style_ttk_widget(self, widget, bg_color, fg_color, state):
        ...
        try:
            ...
            elif widget_class in ['TEntry', 'TSpinbox']:
            ...
            if widget_class == 'TCombobox':
                self.style_combobox(widget, bg_color, fg_color, theme, mode)

        except tk.TclError as error:
            print(f"Error styling {widget_class}: {error}")

    def style_combobox(self, widget, bg_color, fg_color, theme, mode):
        if theme == 'retro':
            field_bg = 'SystemWindow'
            text_color = 'SystemWindowText'
        else:  # Clean theme
            field_bg = 'white' if mode == 'light' else '#181818'
        ...
        widget.option_add('*TCombobox*Listbox.background', field_bg)
        widget.option_add('*TCombobox*Listbox.foreground', text_color)
        widget.option_add('*TCombobox*Listbox.selectBackground', field_bg)
        widget.option_add('*TCombobox*Listbox.selectForeground', text_color)

        # Remove all highlighting
        widget.tk.eval("""
            set popdown [ttk::combobox::PopdownWindow %s]
            $popdown.f.l configure -selectbackground "%s" -selectforeground "%s"
        """ % (widget, field_bg, text_color))
...
class FancyButtonTestApp:
    ...
    def update_app_style(self):
        ...
        for a in self.special_buttons:
            self.recreate_special_buttons(a)
        ...
        self.style.configure('example_label_font12',background=bg_color, foreground=fg_color)
