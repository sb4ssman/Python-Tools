    ...
    def style_descendants(self, widget, colors, mode):
        ...
        for child in widget.winfo_children():
            try:
                ...
                if widget_class.startswith('T'):  # ttk widget
                    self.style_ttk_widget(child, bg_color, fg_color)
                else:  # tk widget
                    self.style_tk_widget(child, bg_color, fg_color)
    ...
    def style_tk_widget(self, widget, bg_color, fg_color):
        ...
        try:
            if widget_class in ['Label', 'Button', 'Radiobutton', 'Checkbutton']:
                ...
                print(f"Unhandled tk widget type: {widget_class}")
        except tk.TclError as e:
            print(f"Error configuring {widget_class}: {e}")

    def style_ttk_widget(self, widget, bg_color, fg_color):
        widget_class = widget.winfo_class()
        font_size = self.get_font_size(widget)
        style_name = f"Custom.{widget_class}.{font_size}"

        try:
            if widget_class in ['TLabel', 'TButton', 'TCheckbutton', 'TRadiobutton']:
                ...
                self.style.configure(style_name, background=bg_color, troughcolor=bg_color)
            elif widget_class in ['Vertical.TScrollbar', 'Horizontal.TScrollbar']:
                self.style.configure(style_name, background=bg_color, troughcolor=bg_color)
            elif widget_class == 'TSeparator':
                # Separators don't typically need styling, but we'll include it for completeness
                pass
            else:
                self.style.configure(style_name, background=bg_color, foreground=fg_color, font=("TkDefaultFont", font_size))
                print(f"Generic styling applied to ttk widget type: {widget_class}")
