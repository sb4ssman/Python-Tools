...
class FancyButton:
    ...
    def style_ttk_widget(self, widget, bg_color, fg_color, state):
        ...
        try:
            ...
            if widget_class == 'TCombobox':
                self.style_combobox(widget, '#ff0000', fg_color, theme, mode)
    ...
    def style_combobox(self, widget, bg_color, fg_color, theme, mode):
        if theme == 'retro':
            # In Retro mode, we don't change the background
            self.style.map('TCombobox', 
                        fieldbackground=[('readonly', "#ffffff")],
                        selectbackground=[('readonly', 'systemHighlight')],
                        selectforeground=[('readonly', 'systemHighlightText')])
        else:
            # In Clean mode, we apply our custom colors
            self.style.map('TCombobox', 
        ...
        self.style.configure('TCombobox', foreground=fg_color, selectforeground=fg_color)
        
        # Apply the styling to the specific widget
